...
#import "NetWorkingPopManager.h"

#define kNOTIFICATION_VIEW_HEIGHT 64

@interface NetWorkingPopManager ()
...
@implementation NetWorkingPopManager
...
+ (void)setOptions:(NSDictionary *)options {
    ...
    if (options[CN_BACKGROUND_COLOR_KEY]) {
        [NetWorkingPopManager shareManager].backgroundColor = options[CN_BACKGROUND_COLOR_KEY];
    }
    
    if (options[CN_TEXT_COLOR_KEY]) {
        [NetWorkingPopManager shareManager].textColor = options[CN_TEXT_COLOR_KEY];
    }
    
    if (options[CN_TEXT_FONT_KEY]) {
        [NetWorkingPopManager shareManager].textFont = options[CN_TEXT_FONT_KEY];
    }
    
    if (options[CN_DELAY_SECOND_KEY]){
        [NetWorkingPopManager shareManager].delaySeconds = [options[CN_DELAY_SECOND_KEY] floatValue];
    }
}

+ (void)showMessage:(NSString *)message {
    
    [NetWorkingPopManager showMessage:message withOptions:nil completeBlock:nil];
}

+ (void)showMessage:(NSString *)message withOptions:(NSDictionary *)options {
    
    [NetWorkingPopManager showMessage:message withOptions:options completeBlock:nil];
}

+ (void)showMessage:(NSString *)message withOptions:(NSDictionary *)options completeBlock:(void(^)())completeBlock {
    
    [NetWorkingPopManager shareManager].completeBlock = completeBlock;
    [NetWorkingPopManager setOptions:options];
    if ([[NetWorkingPopManager shareManager] isShowing]) {
        [[NetWorkingPopManager shareManager] reDisplayTitleLabel:message];
    }else{
        [[NetWorkingPopManager shareManager] showNotification:message];
    }
}
