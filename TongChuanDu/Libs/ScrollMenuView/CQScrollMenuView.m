...
- (void)setTitleArray:(NSArray *)titleArray{
    ...
    [_titleArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (_lastView) {
            menuButton.frame = CGRectMake(_lastView.maxX + 15, 0, 100, self.height);
        }else{
            menuButton.frame = CGRectMake(15, 0, 100, self.height);
        }
        
        menuButton.tag = 100 + idx;
        [menuButton.titleLabel setFont:[UIFont fontWithName:@"PingFangSC-Light" size:15]];
        [menuButton setTitleColor:HexColor(0X333333) forState:UIControlStateNormal];
        [menuButton setTitleColor:HexColor(0X4189F6) forState:UIControlStateSelected];
        if (_showMenuButtonBackGroundImage == YES) {
            [menuButton setBackgroundImage:[UIImage imageNamed:@"App_Market_Home_grayBG_icon"] forState:UIControlStateSelected];
        }
        ...
        if (idx == 0) {
            ...
            _lineView.backgroundColor = HexColor(0X4189F6);
            if (_hideLineView == YES) {
